...
namespace
{
	...
	public:
		...
		explicit wont_compile(size_t v) : value_{ v }{}
		virtual ~wont_compile() = default; // virtual function makes it non-standard layout
...
TEST_CASE("z85.forth_and_badk_and_forth_and_back")
{
	...
	std::ranges::generate(buf, [&rand, &gen]() -> uint8_t { return static_cast<uint8_t>(rand(gen)); });
	...
	std::ranges::for_each(std::views::zip(buf, check), [](auto&& t) -> void { auto [my_b, my_c] { t}; CHECK_EQ(my_b, my_c); });
}
